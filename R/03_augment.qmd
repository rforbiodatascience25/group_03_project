...
#merged_data <- merged_data |> filter(Gene_ID %in% gene_list)

#show(merged_data_clean)
...
if (!("Gene_ID" %in% colnames(merged_data))) {
  stop("The column 'Gene_ID' does not exist in merged_data. Run colnames(merged_data) to inspect the real column names.")
}
