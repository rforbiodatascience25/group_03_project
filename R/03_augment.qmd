...
```{r}
gene_expression <- read_tsv("../data/01_expression_data.tsv")
...
```{r}
pheno_data <- read_tsv("../data/01_phenotype_data.tsv")
```
...
#For each gene
merged_mean <- merged_mean |> 
  mutate(difference_mean_gene_expression = `FALSE`-`TRUE`)
```
...
merged_pheno_control <- merged_pheno_control |> 
  select(Gene_ID, 
         `TRUE`, 
         `FALSE`, 
         difference_mean_gene_expression)
