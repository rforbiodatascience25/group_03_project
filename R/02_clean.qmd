...
```{r}
exprs_data = read_tsv("~/expression_data.tsv")
```
...
write_tsv(expr_clean, "../data/02_gse54514_expression.tsv.gz")

write_tsv(meta_untidy, "../data/02_gse54514_phenotype_untidy.tsv.gz")
...
meta_untidy <- read.table(
  "../data/01_meta_untidy.tsv",
  sep = "\t",
  header = TRUE
)
```
...
Currently, each sample has multiple rows, we want one row per sample and one column per variable. We use pivot_wider()
...
write_tsv(x = platform_table_clean, 
          file = file.path(directory,'data/02_platform_table_clean.tsv'))
```
