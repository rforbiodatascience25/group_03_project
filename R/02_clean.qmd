...
```{r}
platform_table = read_tsv('../data/01_platform_table.tsv')

#Convert platform table into a tibble
platform_table_clean <- tibble(ID = platform_table$ID,
                               ILMN_Gene = platform_table$ILMN_Gene)

#Change the column names
platform_table_clean <- platform_table_clean |> 
  rename("Gene_ID" = ILMN_Gene)

platform_table_clean <- platform_table_clean |> 
  rename("ILMN_Gene" = ID)

write_tsv(x = platform_table_clean, 
          file = "../data/02_platform_table_clean.tsv")
```
