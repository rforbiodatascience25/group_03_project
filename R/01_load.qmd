...
# Asegúrate de que tu línea de escritura de archivo se vea exactamente así:
#write_tsv(x = df_ed,
       #   file = "data/expression_data.tsv")
write_tsv(df_ed, "~/expression_data.tsv")

```

### Loading and saving phenotype data

```{r}
pheno_data <- as_tibble(pheno_data, rownames = "sample_id")
write_tsv(pheno_data, "../data/phenotype_data.tsv")
```

### Loading and saving feature data

```{r}
feature_data <- as_tibble(feature_data, rownames = "probe_id")
write_tsv(feature_data, "../data/feature_data.tsv")
```

### Loading and saving meta data
